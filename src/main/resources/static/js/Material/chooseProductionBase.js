    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getProductionList', json, function(res){
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation"><input type="radio" name="ckitem" id="check'+item.id+'" value="'+item.baseName+'"></td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.baseName+'</td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.baseCode+'</td>';
                            str += '</tr>';
                        }
//                        else{
...
function choose(){
    ...
    parent.$("#baseName").text(productid);
    parent.$("#baseName").val(productid);
    parent.$("#baseName").value=productid;
    parent.$("#baseName").html(productid);
    parent.$("#baseCode").val($("#baseName"+productid).text());
    parent.$("#baseCode").text($("#baseName"+productid).text());
    parent.$("#baseCode").html($("#baseName"+productid).text());
     parent.$("#baseCode").value=$("#baseName"+productid).text();
    //当你在iframe页面关闭自身时
