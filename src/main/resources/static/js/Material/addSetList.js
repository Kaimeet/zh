...
function save(){
    ...
    if($("#companyName").val()==null || $("#companyName").val()=="" || $("#companyName").val()==undefined ){
        layer.msg("请输入账套对应的名称。", {icon : 5, anim : 6, time : 3000});
        return;
    }
    json.companyName = $("#companyName").val();

    if($("#companyCode").val()==null || $("#companyCode").val()=="" || $("#companyCode").val()==undefined ){
         layer.msg("请输入账套对应的代码。", {icon : 5, anim : 6, time : 3000});
         return;
    }
    json.companyCode = $("#companyCode").val();



    // loading层
    var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
    var request = "../ProcessController/saveSetList";
    ajax = $.ajax({
        type : "POST",
...
//                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../ProcessController/setList",{companyName:companyName,companyCode:companyCode,type:$("#type").val()})
//                });
            }
        }
    });
