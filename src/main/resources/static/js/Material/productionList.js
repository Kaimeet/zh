...
function onsearchtable(){
    ...
    var baseCode = $("#baseCode").val().trim();
    if(baseCode!=null && baseCode!="" && baseCode!=undefined){
        json.baseCode = baseCode;
    }
    commonload(json);
}
$('#baseName').bind('keyup', function(event) {
               ...
               　　}
                ...
                $('#baseCode').bind('keyup', function(event) {
                    　　if (event.keyCode == "13") {
                    　　　　//回车执行查询
                    　　　　onsearchtable();
                    　　}
...
function del(id){
    ...
    layer.confirm("确认作废该生产基地吗？", {icon: 3, title:false},function(index1){
        layer.close(index1);
        var json={
...
// loading层
    ...
    var request = "../ProcessController/delProductionList";
    ajax = $.ajax({
        type : "POST",
...
function choose(){
    post("../ProcessController/saveProductionList",{});
}
...
parent.layer.open({
        ...
        area : ['800px', '600px'],
        closeBtn: 1,
        content: '../ProcessController/addProductionList'
    });
}
//编辑
function edit1(id){
parent.layer.open({
        ...
        area : ['800px', '600px'],
        closeBtn: 1,
        content: '../ProcessController/addProductionList?id='+id
    });
}
