...
function save(){
    var json = {};
    json.id = $("#id").val();
    if($("#inventoryName").val()==null || $("#inventoryName").val()=="" || $("#inventoryName").val()==undefined ){
        layer.msg("请输入名称。", {icon : 5, anim : 6, time : 3000});
        return;
    }
    json.inventoryName = $("#inventoryName").val();

    if($("#inventoryCode").val()==null || $("#inventoryCode").val()=="" || $("#inventoryCode").val()==undefined ){
         layer.msg("请输入代码。", {icon : 5, anim : 6, time : 3000});
         return;
    }
    json.inventoryCode = $("#inventoryCode").val();
    json.price = $("#price").val();
    json.longer = $("#longer").val();
    json.wide = $("#wide").val();
    json.thick = $("#thick").val();
    json.mainUnit = $("#mainUnit").val();
    json.subsidiaryUnit = $("#subsidiaryUnit").val();



    // loading层
    var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
    var request = "../ProcessController/saveInventorySelect";
    ajax = $.ajax({
        type : "POST",
        ...
        success : function(result) {
            ...
            if(result.resultCode == 0) {
                ...
                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../ProcessController/inventorySelect",{inventoryCode:inventoryCode,inventoryName:inventoryName,price:price,longer:longer,wide:wide,thick:thick,mainUnit:mainUnit,subsidiaryUnit:subsidiaryUnit,type:$("#type").val()})
                });
            }
        }
    });
