...
function save(){
    ...
    if($("#typeName").val()==null || $("#typeName").val()=="" || $("#typeName").val()==undefined ){
        layer.msg("请输入产品大类对应的名称。", {icon : 5, anim : 6, time : 3000});
        return;
    }
    json.typeName = $("#typeName").val();

    if($("#typeCode").val()==null || $("#typeCode").val()=="" || $("#typeCode").val()==undefined ){
         layer.msg("请输入产品大类对应的代码。", {icon : 5, anim : 6, time : 3000});
         return;
    }
    json.typeCode = $("#typeCode").val();
    json.orgOne=$("#one").val();
    json.orgTwo=$("#two").val();
    json.orgThree=$("#three").val();
    json.orgFour=$("#four").val();



    // loading层
    var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
    var request = "../ProcessController/saveProductCategories";
    ajax = $.ajax({
        type : "POST",
...
//                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../ProcessController/productCategories",{typeName:typeName,typeCode:typeCode,orgOne:orgOne,orgTwo:orgTwo,orgThree:orgThree,orgFour:orgFour,type:$("#type").val()})
//                });
            }
        }
    });
