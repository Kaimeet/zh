...
function save(){
    ...
    if($("#typeCode").val()==null || $("#typeCode").val()=="" || $("#typeCode").val()==undefined ){
         layer.msg("请输入产品分类对应的代码。", {icon : 5, anim : 6, time : 3000});
         return;
    }
    json.typeCode = $("#typeCode").val();
    if($("#typeName").val()==null || $("#typeName").val()=="" || $("#typeName").val()==undefined ){
         layer.msg("请输入产品分类名称。", {icon : 5, anim : 6, time : 3000});
         return;
    }
    ...
    var request = "../ProcessController/saveProductCategoriesII";
    ajax = $.ajax({
        type : "POST",
...
//                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../ProcessController/productCategoriesII",{typeName:$("#typeName").val(),typeCode:$("#typeCode").val(),type:$("#type").val()})
//                });
            }
        }
    });
