...
function choosevarivaluede(variid,varivalue,c,d){
    var columnIndex = parent.$("#chooseIndex").val();
    console.log(bomcode);
    parent.$("#varvalue"+columnIndex).val(varivalue);
    parent.$("#varvalid"+columnIndex).val(variid);

    parent.$("#varvalue").val(varivalue);
    parent.$("#varvalue").text(varivalue);
    parent.$("#varvalue").html(varivalue);
...
function tablestart(){
    ...
    var varivalue=$("#varivalue").val();
    $("#bomtable").bootstrapTable({
        method : 'get',
        url: "../basicFileController/getvarivaluefiles?modelcode="+bomcode+"&varicode="+varicode+"&varivalue="+varivalue,
        ...
        columns:[
            ...
            {
                field:"variablevaule",title:"变量值",align:"center",valign:"middle",sortable:"true",
...
function detailCell(value, row, index) {
    ...
    var d =row.variablevaule;
     var sff= row.id;
    return [
        '<div class="btn-group">',
        '<a onclick="choosevarivaluede('+sff+',\''+d+'\',\''+d+'\','+sff+')"  title="选择" data-toggle="tooltip">选择</a>',
        '</div>'
    ].join('');
};