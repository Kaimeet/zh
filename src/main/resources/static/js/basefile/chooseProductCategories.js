    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getProductCategories', json, function(res){ //getBomStyleList
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation">'+item.id+'</td>';
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose(\''+item.id+'\','+item.userId+',\''+item.typeCode+'\',\''+item.typeName+'\')">选择</a>';
                            str += '</td>';
...
//选择
function choose(id,userId,typeCode,typeName){
    parent.$("#bomcode").val(typeCode);
    parent.$("#bomname").val(typeName);
...
function tablestart(){
    ...
    $("#producetable").bootstrapTable({
        ...
        columns:[
            ...
            {
                field:"typeCode",title:"类型编码",align:"center",valign:"middle",sortable:"true",

            },
            {
                field:"typeName",title:"类型名称",align:"center",valign:"middle",sortable:"true",
