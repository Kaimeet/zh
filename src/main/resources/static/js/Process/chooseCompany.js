    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getCompanyList', json, function(res){//getBOMSeriesList
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation">'+item.createDate+'</td>';
                            str += '<td class="vercenter text-center phoneinformation">';
                            str += '<a style="cursor: pointer;" onclick="choose(\''+item.companyCode+'\',\''+item.companyName+'\')">选择</a><br/>' ;
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
//选择
function choose(companyCode,companyName){
    parent.$("#companyName").val(companyName);
    parent.$("#companyName").text(companyName);
    parent.$("#companyName").html(companyName);
    parent.$("#companyCode").val(companyCode);
    parent.$("#companyCode").text(companyCode);
    parent.$("#companyCode").html(companyCode);
    parent.$("#companyChooseFlag").val(1);
    console.log(parent.$("#companyChooseFlag").val());
