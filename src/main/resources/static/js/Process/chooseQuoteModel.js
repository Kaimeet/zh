...
layui.use(['form','flow','element','layer','laydate'], function() {
    ...
    commonload({typeCode:$("#typeCode").val()});
    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getQuoteModelList', json, function(res){//QuoteModel getBomTypeList
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation">'+item.modelName+'</td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.modelCode+'</td>';
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose('+item.modelCode+','+item.modelName+')">作废</a><br/>' ;
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
function onsearchtable(){
    ...
    if($("#modelName").val()!=null && $("#modelName").val()!="" && $("#modelName").val()!= undefined){
        json.modelName = $("#modelName").val();
    }
    //大类编号
    if($("#typeCode").val()!=null && $("#typeCode").val()!="" && $("#typeCode").val()!= undefined){
        json.typeCode = $("#typeCode").val();
    }
    commonload(json);
}
//重置
...
//选择
function choose(modelCode,modelName){
    parent.$("#modelCode").text(modelCode);
    parent.$("#modelName").val(modelName);
