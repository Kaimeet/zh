    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getBOMForProcessList', json, function(res){
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation">'+item.type+'</td>';
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose('+item.bomCode+','+item.bomName+','+item.type+')">作废</a><br/>' ;
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
function onsearchtable(){
    ...
    var bomStyle=$("#bomStyle").val();
    if(bomStyle!=null && bomStyle!="" && bomStyle!=undefined){
        json.bomStyle=bomStyle;
    }
    var bomType=$("#bomType").val();
    if(bomType!=null && bomType!="" && bomType!=undefined){
        json.bomType=bomType;
    }
    console.log(bomName);
    commonload(json);
}
...
//选择
function choose(bomCode,bomName,type){
    parent.$("#templateCode").val(bomCode);
    parent.$("#templateName").val(bomName);
