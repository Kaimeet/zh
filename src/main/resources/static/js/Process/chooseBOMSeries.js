    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../ProcessController/getBOMSeriesList', json, function(res){
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose('+item.seriesCode+','+item.seriesName+')">作废</a><br/>' ;
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
//选择
function choose(seriesCode,seriesName){
    parent.$("#seriesCode").val(seriesCode);
    parent.$("#seriesName").val(seriesName);
