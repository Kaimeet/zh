                ...
                $.get('../ProcessController/getUserList', json, function(res){ //getBomStyleList
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose(\''+item.employeeName+'\',\''+item.userId+'\',\''+item.departmentName+'\')">选择</a>';
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
//选择
function choose(employeeName,userId,departmentName){
    console.log(employeeName);
    parent.$("#employeeName").val(employeeName);
//    parent.$("#employeeName").text(employeeName);
//    parent.$("#employeeName").html(employeeName);
    parent.$("#userId").val(userId);
    parent.$("#userId").text(userId);
    parent.$("#userId").html(userId);
    parent.$("#departmentName").val(departmentName);
//    parent.$("#departmentName").text(departmentName);
//    parent.$("#departmentName").html(departmentName);
