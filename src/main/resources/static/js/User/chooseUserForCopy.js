    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../UserController/getChooseUserForCopyList', json, function(res){//getBOMSeriesList
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<a style="cursor: pointer;" onclick="choose( \''+item.copyUserId+'\',\''+item.employeeName+'\')">选择</a><br/>' ;
                            str += '</td>';
                            str += '</tr>';
                        }
                        lis.push(str);
...
function onsearchtable(){
    ...
    if($("#departmentNameOne").val()!=null && $("#departmentNameOne").val()!="" && $("#departmentNameOne").val()!= undefined){
        json.departmentNameOne = $("#departmentNameOne").val();
    }
    if($("#departmentNameTwo").val()!=null && $("#departmentNameTwo").val()!="" && $("#departmentNameTwo").val()!= undefined){
        json.departmentNameTwo = $("#departmentNameTwo").val();
    }
    commonload(json);
}
//重置
...
//选择
function choose(copyUserId,copyForName){
    parent.$("#copyUserId").text(copyUserId);
    parent.$("#copyUserId").val(copyUserId);
    parent.$("#copyForName").val(copyForName);
    parent.$("#copyForName").val(copyForName);
