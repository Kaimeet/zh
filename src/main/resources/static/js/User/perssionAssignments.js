...
function save(){
    ...
    if(barlength==0){
        layer.msg("请为该用户分配权限。", {icon : 5, anim : 6, time : 3000});
        return ;
    }

    $("#bar").children("li").each( function(){ var ids= $(this).attr("id"); menuIds.push(ids); });


    json.menuIds = menuIds;
    var id = $("#id").val();
    if(id==null || id == "" || id==undefined){
        layer.msg("要分配权限的角色不存在。", {icon : 5, anim : 6, time : 3000});
        return ;
    }
    json.id = id;
    // loading层
    var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
    var request = "../UserController/savePressiom";
    ajax = $.ajax({
        type : "POST",
...
//                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../UserController/perssionAssignments",{id:result.id})
//                });
            }
        }
    });
