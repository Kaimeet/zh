                ...
                $.get('../SystemController/getSystemNotificationList', json, function(res){
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation">'+item.title+'</td>';
...
//新增
function add(){
    post("../SystemController/addAndEditNotice",{})
}
//编辑
function edit(id){
    console.log(id);
    post("../SystemController/addAndEditNotice",{id:id})
}
...
function releaseto(id){
    ...
    if(id==null ||  id=="" ||  id==undefined){
        layer.msg("消息通知信息丢失，请刷新页面之后再试。", {icon : 5, anim : 6, time : 3000});
    }
    var json = {"id":id,"releaseFalg":1};
    // loading层
    var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
    var request = "../SystemController/releaseNotice";
    ajax = $.ajax({
        type : "POST",
...
function releaseback(id){
    //产品表id
    if(id==null ||  id=="" ||  id==undefined){
        layer.msg("消息公告信息丢失，请刷新页面之后再试。", {icon : 5, anim : 6, time : 3000});
    }
...
function del(id){
    ...
    layer.confirm("确认作废该条消息通知吗？", {icon: 3, title:false},function(index, layero){
        var json = {"id":id};
        // loading层
        var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
        var request = "../SystemController/delNotice";
        ajax = $.ajax({
            type : "POST",
            url : encodeURI(request),
            data : json,
