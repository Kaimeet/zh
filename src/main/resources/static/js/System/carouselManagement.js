    ...
    flow.load({
        ...
        ,done : function(page, next) { //执行下一页的回调
            ...
            setTimeout(function(){
                ...
                $.get('../SystemController/getCarouselManagementList', json, function(res){
                    ...
                    layui.each(obj.date, function(index, item){
                        ...
                        if($("#bwType").val()==0){//电脑版
                            ...
                            str += '<td class="vercenter text-center phoneinformation"><img src="'+item.viewUrl+'" class="imgshow" alt="" height="40px" width="120px" ></td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.carouselTitle+'</td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.releaseFalg+'</td>';
                            str += '<td class="vercenter text-center phoneinformation">'+item.showOrder+'</td>';
                            ...
                            if(item.releaseFalg=="已发布"){
                                str +='<a style="cursor: pointer;" onclick="releaseback('+item.id+')">撤回</a><br/><a style="cursor: pointer;" onclick="edit('+item.id+')">查看</a>' ;
                            }else{
                                str +='<a style="cursor: pointer;" onclick="releaseto('+item.id+')">发布</a><br/><a style="cursor: pointer;" onclick="edit('+item.id+')">编辑</a><br/><a style="cursor: pointer;" onclick="del('+item.id+')">作废</a>' ;
                            }
                            str +='</td>';
                            str += '</tr>';
                        }else{
                            ...
                            str+='</div>';
                            str+='<div class="row" style="margin:5px 0;">';
                            str+='<div class="col-xs-4 text-right" style="white-space:nowrap;">轮播图</div>';
                            str+='<div class="col-xs-8 text-left phoneinformation">'+item.carouselTitle+'</div>';
                            str+='</div>';
                            str+='<div class="row" style="margin:5px 0;">';
                            str+='<div class="col-xs-4 text-right" style="white-space:nowrap;">轮播图主题</div>';
                            str+='<div class="col-xs-8 text-left phoneinformation">'+item.carouselTitle+'</div>';
                            str+='</div>';
                            str+='<div class="row" style="margin:5px 0;">';
                            str+='<div class="col-xs-4 text-right" style="white-space:nowrap;">是否发布</div>';
                            str+='<div class="col-xs-8 text-left phoneinformation">'+item.releaseFalg+'</div>';
                            str+='</div>';
                            str+='<div class="row" style="margin:5px 0;">';
                            str+='<div class="col-xs-4 text-right" style="white-space:nowrap;">显示顺序</div>';
                            str+='<div class="col-xs-8 text-left phoneinformation">'+item.showOrder+'</div>';
                            str+='</div>';
                            str+='<div class="row" style="margin:5px 0;">';
                            ...
                            if(item.releaseFalg=="已发布"){
                                str+='<a  style="cursor: pointer;" onclick="releaseback('+item.id+')">撤回</a>' ;
                                str+='&nbsp&nbsp&nbsp';
                                str+='<a style="cursor: pointer;" onclick="edit('+item.id+')">查看</a>';
                            }else{
                                str+='<a  style="cursor: pointer;" onclick="releaseto('+item.id+')">发布</a>' ;
                                str+='&nbsp&nbsp&nbsp';
...
function del(id){
    ...
    layer.confirm("确认作废该条轮播图记录吗？", {icon: 3, title:false},function(index, layero){
        var json = {"id":id};
        // loading层
        var index = layer.load(1, {shade: [0.5,'#fff']}); // 0.5透明度的白色背景
...
//新增
function add(){
    post("../SystemController/addAndEditCarousel",{})
}

//编辑
function edit(id){
    post("../SystemController/addAndEditCarousel",{id:id})
}
