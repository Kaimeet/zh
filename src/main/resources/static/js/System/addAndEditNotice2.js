...
$(function() {
    ...
    $('#edit').on('froalaEditor.initialized', function (e, editor) {
        ...
        .froalaEditor({
            ...
            saveURL: '../SystemController/saveNoticArticle',  //自动保存的地址（body参数为html内容）  saveProductArticle
            saveParams: { id: id},  //保存内容时传的参数
            // spellcheck: false,  //是否允许浏览器对输入内容进行拼写检查
            imageUploadURL: '../SystemController/uploadArticleImg',//上传到本地服务器                  uploadArticleImg
...
function saveArticle(releaseFalg){
    ...
    $.ajax({
        type: 'POST',
        url: '../SystemController/saveNoticArticle',
        dataType: 'json',
        data:{
            "body":html,
            "id":$("#id").val(), //产品表ID
...
//                layer.msg(result.resultMassage, {icon : 6, anim : 5, time : 3000},function(){
                    post("../SystemController/addAndEditNotice2",{"id":$("#id").val()})
//                });
            }
...
//返回上一部
function back(){
    post("../SystemController/addAndEditNotice",{"id":$("#id").val()})
}
